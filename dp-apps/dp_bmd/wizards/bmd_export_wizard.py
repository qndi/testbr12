...
class BmdExportWizard(models.TransientModel):
    ...
    def do_export(self):
        ...
        filename = '%s_%s.csv' % ('out_invoice', timestamp)
        ...
        if self.type == 'unexported':
            bmd_invoices = self.env['account.invoice'].search(
                [('bmd_export', '=', True), ('type', 'in', ['out_refund', 'out_invoice']), ('state', 'in', ['open', 'paid'])])
        elif self.type == 'date_range':
            bmd_invoices = self.env['account.invoice'].search(
                [('date_invoice', '<', self.date_end), ('date_invoice', '>', self.date_begin),
                 ('type', 'in', ['out_refund', 'out_invoice']), ('state', 'in', ['open', 'paid'])])
        if bmd_invoices:
    ...
    def get_row_faktura(self, inv, line):
        ...
        faktura_dict['konto'] = inv.partner_id.ref
        faktura_dict['gkonto'] = line.account_id.code
        ...
        end_of_month = inv.date_invoice.replace(day=calendar.monthrange(inv.date_invoice.year, inv.date_invoice.month)[1])
        ...
        if inv.type == 'out_invoice':
            ...
            faktura_dict['buchungssymbol'] = 'ER'
        elif inv.type == 'in_refund':
            faktura_dict['buchungssymbol'] = 'EG'
    ...
    def get_row_customer(self, customer):
        ...
        customers_dict['ktonr'] = customer.ref
